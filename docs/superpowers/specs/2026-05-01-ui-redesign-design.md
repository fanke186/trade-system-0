# UI Redesign · TokyoNightStorm 工业终端风

## 概述

将 trade-system-0 前端从通用浅色 Inter 配合蓝色点缀的方案，重新设计为基于 TokyoNightStorm 配色 + DM Mono/Sans 字体的工业终端风格暗色方案。核心理念：**"每个像素都在传递信息"** ——颜色是语义信号系统，字体定义信息层级，不纯为装饰。

## 设计决策

### 配色方案：TokyoNightStorm

基于 TokyoNightStorm iTerm 配色方案，低对比、护眼、适合长时间盯盘。

| Token | Hex | 用途 |
|-------|-----|------|
| `--background` | `#24283b` | 页面底色 |
| `--panel` | `#1d202f` | 侧栏、面板底色 |
| `--border` | `#364a82` | 边框、分隔线 |
| `--foreground` | `#c0caf5` | 主文字色 |
| `--muted` | `#2f3548` | 次要背景 |
| `--muted-foreground` | `#565f89` | 次要文字 |
| `--success` / buy | `#9ece6a` | 买入信号、成功状态 |
| `--danger` / sell | `#f7768e` | 卖出信号、危险操作 |
| `--warning` / hold | `#e0af68` | 观望、警告状态 |
| `--accent` | `#7aa2f7` | 主强调色 |
...
| **Panel** | 1px border 色边框，顶部标题栏与内容区间用 border 分隔。可选顶部 1px accent 发光线 |
...
| 扫描线纹理 | 全局可选 overlay：1px 重复半透明黑线，模拟 CRT 屏幕质感，设置中可关闭 |
...
| `src/components/shared/Panel.tsx` | 暗色边框 + 顶部分隔 |
| `src/components/shared/DataTable.tsx` | 暗色行 hover + monospace 数字列 |
| `src/components/shared/Field.tsx` | 下划线式 input/select/textarea |
| `src/pages/*.tsx` | 各页面暗色适配（根据需要微调） |

### 不做的

- 不动 KLineChart 图表库内部渲染（第三方库，只改容器）
