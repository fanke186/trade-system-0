...
pub fn sync_kline(conn: &DuckConnection, stock_code: &str, mode: &str) -> AppResult<KlineSyncResult> {
    if !matches!(mode, "full" | "incremental") {
        return Err(AppError::new("kline_sync_failed", "mode 只允许 full 或 incremental", true));
    }
...
fn sync_inner(conn: &DuckConnection, stock_code: &str, mode: &str) -> AppResult<KlineSyncResult> {
    ...
    let (start_date, end_date) = choose_sync_range(conn, symbol_id, mode)?;
...
pub fn aggregate_kline(
    ...
    if !matches!(frequency, "1w" | "1M") {
        return Err(AppError::new("invalid_frequency", "聚合只允许 1w 或 1M", true));
    }
...
fn ensure_security(conn: &DuckConnection, stock_code: &str) -> AppResult<i64> {
    ...
    let exchange = if stock_code.starts_with('6') { "SH" } else { "SZ" };
    conn.execute(
